...
// GET /api/roles — ambil semua role config + user count per role
export async function GET() {
  try {
    const [roles, ...roleCounts] = await Promise.all([
      prisma.roleConfig.findMany({ orderBy: { createdAt: "asc" } }),
      // Hitung user per Role enum secara paralel
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await prisma.roleConfig.findUnique({ where: { name: name.trim() } });
    if (existing) {
